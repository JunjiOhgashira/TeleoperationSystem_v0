...
{
    //車両前方左の進路軌跡を描画するクラス
    public class Left_traj : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            double[,] pre_trj_left = new double[count, 3];
            Vector3 v_pre_trj_left = new Vector3();
            line.positionCount = count;

            //重心の予測表示座標点から車両前方左が描く進路軌跡を計算し仮想環境上に描画する
            for (int m = 0; m < count; m++)
            {

                pre_trj_left[m, 0] = all_prepoint_x[m] - center_side * Math.Cos(all_yaw_angle[m]) + center_long * Math.Sin(all_yaw_angle[m]);
                pre_trj_left[m, 1] = 0;
                pre_trj_left[m, 2] = all_prepoint_z[m] + center_long * Math.Cos(all_yaw_angle[m]) + center_side * Math.Sin(all_yaw_angle[m]);
                v_pre_trj_left.x = (float)pre_trj_left[m, 0];
                v_pre_trj_left.y = (float)pre_trj_left[m, 1];
                v_pre_trj_left.z = (float)pre_trj_left[m, 2];
                line.SetPosition(m, v_pre_trj_left);
            }
        }
    }
}
